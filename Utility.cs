...
namespace MatchZy
{
    public partial class MatchZy
    {
        ...
        private void ExecWarmupCfg() {
            ...
            {
                var addlPath = Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlWarmupConfigs[i]);
                if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlWarmupConfigs[i])))
                {
                    Log($"[StartWarmup] Executing additional Warmup CFG from {addlPath}");
                    Server.ExecuteCommand($"exec {addlPath}");
                } 
                else
                {
                    Log($"[StartWarmup] Additional Warmup CFG not found in {addlPath}, skipping!");
                }
            }
            ...
            if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg", warmupCfgPath))) {
                ...
                Server.ExecuteCommand($"exec {warmupPath}");
            } else {
                Log($"[StartWarmup] Starting warmup! Warmup CFG not found in {warmupPath}, using default CFG!");
        ...
        private void StartKnifeRound() {
            ...
            {
                var addlPath = Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlKnifeConfigs[i]);
                if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlKnifeConfigs[i])))
                {
                    Log($"[StartKnifeRound] Executing additional Warmup CFG from {addlPath}");
                    Server.ExecuteCommand($"exec {addlPath}");
                }
                else
                {
                    Log($"[StartKnifeRound] Additional Warmup CFG not found in {addlPath}, skipping!");
                }
            }
            ...
            if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg", knifeCfgPath))) {
                ...
                Log($"[StartKnifeRound] Starting Knife! Knife CFG not found in {knifePath}, using default CFG!");
        ...
        private void HandleMatchStart() {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Green}FragBot{ChatColors.Default} Plugin by {ChatColors.Green}sphaxa{ChatColors.Default}");
        }
        ...
        private void ExecLiveCFG()
        {
            ...
            {
                var addlPath = Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlLiveConfigs[i]);
                if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg/MatchZy", addlLiveConfigs[i])))
                {
                    Log($"[StartLive] Executing additional Warmup CFG from {addlPath}");
                    Server.ExecuteCommand($"exec {addlPath}");
                }
                else
                {
                    Log($"[StartLive] Additional Warmup CFG not found in {addlPath}, skipping!");
                }
            }
            ...
            if (File.Exists(Path.Join(Server.GameDirectory + "/csgo/cfg", liveCfgPath))) {
                ...
                Log($"[StartLive] Starting Live! Live CFG not found in {livePath}, using default CFG!");
